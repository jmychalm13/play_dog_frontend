    ...
    <section className="bg-gradient-to-b from-gray-300 to-green-800">
      ...
      <div className="cards grid grid-cols-3 gap-4">
        {users.map((user) => (
          <div
            key={user.id}
            className="flex items-center justify-center p-5 bg-gray-600 border border-gray-700 rounded-lg shadow-xl"
          >
            <div className="text-center max-w-screen-md">
              ...
              <h5 className="text-xl text-white">Pets</h5>
              {user.dogs.map((dog) => (
                <div key={dog.id} className="text-white">
                  <p>{dog.name}</p>
                  <p>{dog.breed}</p>
